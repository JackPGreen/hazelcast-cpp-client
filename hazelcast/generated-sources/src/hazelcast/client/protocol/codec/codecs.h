            ...
            namespace codec {
                ...
                struct HAZELCAST_API client_addclusterviewlistener_handler : public impl::BaseEventHandler {
                    ...
                    */
                     ...
                     * @param member_groups Grouped members by their UUID. Grouping is done based on RoutingStrategy.
                    */
                    virtual void handle_membergroupsview(int32_t version, std::vector<std::vector<boost::uuids::uuid>> const &member_groups) = 0;
                 ...
                 */
                ClientMessage HAZELCAST_API client_sendschema_encode(const serialization::pimpl::schema &schema);

                /**
                 * Fetches a schema from the cluster with the given schemaId
                 */
                ClientMessage HAZELCAST_API client_fetchschema_encode(int64_t schema_id);

                /**
                 * Sends all the schemas to the cluster
                 */
                ClientMessage HAZELCAST_API client_sendallschemas_encode(const std::vector<serialization::pimpl::schema> &schemas);
                 ...
                 */
                ClientMessage HAZELCAST_API sql_close_encode(const sql::impl::query_id &query_id);

                /**
                 * Starts execution of an SQL query (as of 4.2).
                 */
                ClientMessage HAZELCAST_API sql_execute_encode(const std::string &sql, const std::vector<serialization::pimpl::data> &parameters, int64_t timeout_millis, int32_t cursor_buffer_size, const std::string *schema, byte expected_result_type, const sql::impl::query_id &query_id, bool skip_update_statistics);

                /**
                 * Fetches the next row page.
                 */
                ClientMessage HAZELCAST_API sql_fetch_encode(const sql::impl::query_id &query_id, int32_t cursor_buffer_size);
